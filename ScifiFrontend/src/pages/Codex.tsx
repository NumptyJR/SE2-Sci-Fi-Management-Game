    ...
    <div className="min-h-screen bg-background bg-grid-sci-fi">
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground mt-1">Lore and reference for the sovereign system.</p>
        </div>
            ...
            >
              ...
              >
                <CardHeader className="pb-2 flex flex-row items-center gap-2">
                  ...
                  <CardTitle className="text-lg text-cyan-100">{entry.title}</CardTitle>
                </CardHeader>
              </button>
              {expandedId === entry.id && (
                  ...
                  {entry.children && entry.children.length > 0 && (
                    <div className="mt-4 space-y-3 border-t border-border pt-4">
                      {entry.children.map((child) => (
                        <div key={child.id}>
                          <p className="font-medium text-cyan-200 text-sm">{child.title}</p>
                          <p className="text-sm text-muted-foreground mt-0.5">{child.content}</p>
                        </div>
                      ))}
                    </div>
                  )}
